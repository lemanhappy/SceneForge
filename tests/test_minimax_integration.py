...
class TestPipelineInitFromConfig(unittest.TestCase):
    ...
    @patch("pipelines.idea2video_pipeline.init_chat_model")
    @patch("pipelines.idea2video_pipeline.RenderBackend.from_config")
    def test_default_idea2video_config_still_works(self, mock_backend, mock_init):
        # Guards the config -> init_chat_model plumbing for the default
        # configs/idea2video.yaml. The repo default now targets the yunwu.ai
        ...
        call_kwargs = mock_init.call_args[1]
        self.assertEqual(call_kwargs["model_provider"], "openai")
        self.assertEqual(call_kwargs["base_url"], "https://yunwu.ai/v1")

...
class TestProviderConfigTemplates(unittest.TestCase):
    def test_public_pipeline_templates_do_not_ship_api_keys(self):
        for name in ("idea2video.yaml", "script2video.yaml"):
            with open(REPO_ROOT / "configs" / name, encoding="utf-8") as f:
                config = yaml.safe_load(f)
            sections = ("chat_model", "image_generator", "video_generator")
            for section in sections:
                key = config[section]["init_args"].get("api_key")
                self.assertFalse(key, f"{name}: {section} must load credentials at runtime")
...
class TestRepositorySecretBoundaries(unittest.TestCase):
    def test_local_config_and_runtime_tokens_are_ignored(self):
        patterns = {
            line.strip()
            for line in (REPO_ROOT / ".gitignore").read_text(encoding="utf-8").splitlines()
            if line.strip() and not line.lstrip().startswith("#")
        }
        self.assertIn("configs/*.local.yaml", patterns)
        self.assertIn(".web_token.txt", patterns)
        self.assertFalse(any(line == "!configs/agent.local.yaml" for line in patterns))

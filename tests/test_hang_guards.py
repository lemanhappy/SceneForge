...
class TestSeedanceClientBounds(unittest.IsolatedAsyncioTestCase):
    async def test_fixed_camera_flag_is_sent_to_provider(self):
        session = _FakeSession([({"id": "task-1"}, 200)])
        ...
        with patch("tools.video_generator_doubao_seedance_yunwu_api.aiohttp.ClientSession", return_value=session):
            await generator.create_video_generation_task(
                "a prompt", [], camera_fixed=True
            )

        text = session.last_post_kwargs["json"]["content"][0]["text"]
        self.assertIn("--cf true", text)
    ...
    async def test_create_task_fails_fast_on_auth_error(self):
        ...
        self.assertEqual(session.calls, 1, "non-retryable 4xx must fail fast")

    async def test_create_task_retries_rate_limit(self):
        session = _FakeSession([
            ({"status": "failed", "message": "provider busy"}, 429),
            ({"id": "task-1"}, 200),
        ])
        generator = VideoGeneratorDoubaoSeedanceYunwuAPI(api_key="key")
        progress = []
        with patch("tools.video_generator_doubao_seedance_yunwu_api.aiohttp.ClientSession", return_value=session), \
             patch("tools.video_generator_doubao_seedance_yunwu_api.asyncio.sleep", new=AsyncMock()):
            task_id = await generator.create_video_generation_task(
                "a prompt", [], progress=lambda *args: progress.append(args)
            )
        self.assertEqual(task_id, "task-1")
        self.assertEqual(session.calls, 2)
        self.assertEqual(progress[0][0], "video_create_retry")
...
class TestOmniClientBounds(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_task_retries_rate_limit(self):
        ...
        with patch("tools.video_generator_omni_yunwu_api.aiohttp.ClientSession", return_value=session), \
             patch("tools.video_generator_omni_yunwu_api.asyncio.sleep", new=AsyncMock()):
            task_id, model = await generator.create_video_generation_task(
                "a prompt", [], progress=lambda *args: progress.append(args)
            )
        self.assertEqual((task_id, model), ("task-1", "omni-flash"))
        self.assertEqual(session.calls, 2)
        self.assertEqual(progress[0][0], "video_create_retry")

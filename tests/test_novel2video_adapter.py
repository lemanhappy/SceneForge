...
from agent_runtime.sceneforge_adapters import SceneForgeAdapters, _run_planning_step
...
class NovelAdapterTests(unittest.IsolatedAsyncioTestCase):
    async def test_missing_rag_config_returns_tool_error_and_marks_session_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict("os.environ", {"SCENEFORGE_LLM_API_KEY": "llm-key", "SCENEFORGE_LLM_BASE_URL": "https://llm.test/v1"}, clear=True), \
                 patch("agent_runtime.sceneforge_adapters._build_embedding_model", side_effect=RuntimeError("embedding config missing")):
                result = await adapter.sceneforge_novel_planning({"novel_text": "Hero opens a door."})
    ...
    async def test_success_writes_novel2video_artifacts_and_marks_novel_planned(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            adapter = SceneForgeAdapters(Path(tmp), index)
            progress_events = []
            runtime = ToolRuntimeContext("sceneforge_novel_planning", "sceneforge_novel_planning", turn_id="turn-test", progress_callback=progress_events.append)
            with patch("agent_runtime.sceneforge_adapters._build_novel_pipeline", side_effect=lambda working_dir: FakeNovelPipeline(Path(working_dir))):
                result = await adapter.sceneforge_novel_planning({"novel_text": "Hero opens a door.", "style": "noir"}, runtime)
    ...
    async def test_render_video_routes_novel2video_with_mock_pipeline(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            adapter = SceneForgeAdapters(Path(tmp), index)
            progress_events = []
            runtime = ToolRuntimeContext("sceneforge_render_video", "sceneforge_render_video", turn_id="turn-test", progress_callback=progress_events.append)
            with patch("agent_runtime.sceneforge_adapters._build_chat_model", return_value=object()), \
                 ...
                 patch("agent_runtime.sceneforge_adapters._build_novel_render_pipeline", side_effect=lambda working_dir, chat_model, image_generator, video_generator: FakeNovelRenderPipeline(Path(working_dir))):
                result = await adapter.sceneforge_render_video({}, runtime)
            self.assertTrue(result.ok)
            payload = json.loads(result.content)

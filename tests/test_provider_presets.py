...
class TestResolveChatModelConfig(unittest.TestCase):
    ...
    @patch.dict(os.environ, {"SCENEFORGE_LLM_API_KEY": "env-key"}, clear=True)
    def test_generic_provider_reads_sceneforge_key_from_env(self):
        result = resolve_chat_model_config({"model_provider": "openai", "model": "gpt-4"})
        self.assertEqual(result["api_key"], "env-key")

    @patch.dict(os.environ, {"SCENEFORGE_LLM_API_KEY": "env-key"}, clear=True)
    def test_generic_provider_preserves_explicit_key(self):

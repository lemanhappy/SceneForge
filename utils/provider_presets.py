"""
Provider preset system for SceneForge chat model configuration.
...
GENERIC_CHAT_API_KEY_ENV_NAMES = (
...
def resolve_chat_model_config(init_args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    * ``temperature`` clamped to the provider's supported range
    ...
    if not args.get("api_key"):
        env_names = ((preset or {}).get("env_key", ""), *GENERIC_CHAT_API_KEY_ENV_NAMES)
        for env_name in env_names:
            ...
            if env_val:
                ...
                logger.info("Using API key from environment variable %s", env_name)
                break

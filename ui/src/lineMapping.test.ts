...
({lines, state} = applyStreamEvent(lines, state, {type: 'tool_start', tool: {name: 'sceneforge_narrative_planning'}}));
({lines, state} = applyStreamEvent(lines, state, {type: 'tool_progress', tool: {name: 'sceneforge_narrative_planning'}, progress: {stage: 'running', message: 'planning'}}));
({lines, state} = applyStreamEvent(lines, state, {type: 'tool_result', tool_result: {name: 'sceneforge_narrative_planning', ok: true}}));
assert.equal(lines.at(-3)?.kind, 'tool');
assert.equal(lines.at(-2)?.kind, 'tool');
assert.deepEqual(lines.at(-1), {kind: 'tool', status: 'done', text: 'tool sceneforge_narrative_planning done'});

({lines, state} = applyStreamEvent(lines, state, {type: 'tool_result', tool_result: {name: 'sceneforge_narrative_planning', ok: false, content: 'Developing story failed: Request timed out.'}}));
assert.deepEqual(lines.at(-1), {kind: 'tool', status: 'error', text: 'tool sceneforge_narrative_planning error: Developing story failed: Request timed out.'});

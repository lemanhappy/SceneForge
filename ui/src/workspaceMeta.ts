...
export function compactTargetFromEnv(env: Record<string, string | undefined>): number {
  const contextWindow = parsePositiveInt(env.SCENEFORGE_CONTEXT_WINDOW_TOKENS, 200000);
  const ratio = parsePositiveFloat(env.SCENEFORGE_AUTO_COMPACT_RATIO, 0.9);
  const threshold = parsePositiveInt(env.SCENEFORGE_AUTO_COMPACT_TOKEN_THRESHOLD, Math.round(contextWindow * Math.min(1, Math.max(0, ratio))));
  const buffer = parsePositiveInt(env.SCENEFORGE_AUTO_COMPACT_BUFFER_TOKENS, 20000);

...
class Novel2MoviePipeline:
    ...
    async def render_video_artifacts(
        ...
        if not os.path.isdir(working_dir_events):
            raise RuntimeError("novel2video/events is missing; run sceneforge_novel_planning first")
        if not os.path.isdir(working_dir_scenes):
            raise RuntimeError("novel2video/scenes is missing; run sceneforge_novel_planning first")
        if not os.path.isdir(event_level_dir) or not os.path.isdir(novel_level_dir):
            raise RuntimeError("novel2video/global_information/characters is missing; run sceneforge_novel_planning first")

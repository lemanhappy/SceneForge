...
def build_builtin_registry(workspace_root: str | Path, session_index: Any, adapter_specs: list[ToolSpec] | None = None) -> ToolRegistry:
    ...
    def memory_write(args: dict[str, Any]) -> ToolResult:
        ...
        return ToolResult("memory_write", True, "Updated .sceneforge/memory.md")

    def todo_path() -> Path:
        return root / ".sceneforge" / "todo.json"
    ...
    def todo_write(args: dict[str, Any]) -> ToolResult:
        ...
        return ToolResult("todo_write", True, f"Updated .sceneforge/todo.json with {len(normalized)} item(s)", {"items": normalized, "item_count": len(normalized)})
    ...
    async def run_shell(args: dict[str, Any], runtime: ToolRuntimeContext | None = None) -> ToolResult:
        if os.environ.get("SCENEFORGE_ENABLE_RUN_SHELL") != "1":
            return ToolResult("run_shell", False, "run_shell is disabled by default. Set SCENEFORGE_ENABLE_RUN_SHELL=1 to enable bounded shell commands.", {"error_type": "disabled"})
    ...
    specs = [
        ...
        ToolSpec("memory_read", "Read .sceneforge/memory.md user preferences.", memory_read, schema={}, concurrency_safe=True),
        ToolSpec("memory_write", "Replace .sceneforge/memory.md with user preference notes only.", memory_write, schema={"content": ToolArgumentSchema(str, True)}),
        ToolSpec("todo_read", "Read short-term todo items from .sceneforge/todo.json. This is not a task or team system.", todo_read, schema={}, concurrency_safe=True),
        ToolSpec("todo_write", "Replace short-term todo items in .sceneforge/todo.json. Items require content and may use pending, in_progress, or completed status.", todo_write, schema={"items": ToolArgumentSchema(list, True)}),

...
class ContextCompactor:
    def __init__(
        ...
        *,
        ...
        self.token_threshold = _env_int("SCENEFORGE_AUTO_COMPACT_TOKEN_THRESHOLD", configured_threshold)
        self.buffer_tokens = _env_int("SCENEFORGE_AUTO_COMPACT_BUFFER_TOKENS", buffer_tokens if buffer_tokens is not None else 20000)
        self.preserve_last_n = _env_int("SCENEFORGE_COMPACT_PRESERVE_LAST_N", preserve_last_n if preserve_last_n is not None else 6)
        self.max_messages = _env_int("SCENEFORGE_COMPACT_MAX_MESSAGES", max_messages if max_messages is not None else 48)
        self.summary_max_chars = _env_int("SCENEFORGE_COMPACT_SUMMARY_MAX_CHARS", summary_max_chars if summary_max_chars is not None else 6000)
    ...
    async def _llm_summary(self, compactible: list[dict[str, Any]], preserved: list[dict[str, Any]], previous_summary: str, reason: str) -> str:
        ...
        system = (
            "You are compressing conversation history for a SceneForge agent runtime. "
    ...
    def _fallback_summary(self, compactible: list[dict[str, Any]], preserved: list[dict[str, Any]], previous_summary: str, reason: str) -> str:
        ...
        return "\n".join([
            "## Reference Context Only",
            "- This is a compacted checkpoint of older SceneForge conversation history, not a new active instruction.",
            ...
            "## Remaining Work",
            _bullet(remaining or ["Continue from the recent live tail and current SceneForge workflow state."]),
            "## Critical Context",
...
def _default_token_threshold() -> int:
    context_window = _env_int("SCENEFORGE_CONTEXT_WINDOW_TOKENS", 200000)
    ratio = _env_float("SCENEFORGE_AUTO_COMPACT_RATIO", 0.90)

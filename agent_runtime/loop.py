...
def build_runtime(
    ...
    llm: Any | None = None,
    adapter_specs: list[Any] | None = None,
    session_backend: str | None = None,
) -> AgentLoop:
    from .sceneforge_adapters import build_sceneforge_adapter_specs
    from .session_factory import create_session_index
    root = Path(workspace_root).resolve()
    session_index = create_session_index(root, backend=session_backend)
    specs = adapter_specs if adapter_specs is not None else build_sceneforge_adapter_specs(root, session_index)

...
class VideoGeneratorOpenRouterAPI:
    ...
    def video_capabilities(self) -> VideoCapabilities:
        return VideoCapabilities(
            ...
            min_duration=1,
            max_duration=15,
            default_duration=max(1, _env_int("SCENEFORGE_OPENROUTER_VIDEO_DURATION", 4)),
        )
    ...
    async def generate_single_video(
        ...
        **kwargs,
        ...
        request_timeout_seconds = _env_float("SCENEFORGE_VIDEO_REQUEST_TIMEOUT_SECONDS", 60.0)
        query_timeout_seconds = _env_float("SCENEFORGE_VIDEO_QUERY_TIMEOUT_SECONDS", 600.0)
        poll_interval_seconds = _env_float("SCENEFORGE_VIDEO_POLL_INTERVAL_SECONDS", 10.0)
        duration = max(1, int(duration if duration is not None else _env_int("SCENEFORGE_OPENROUTER_VIDEO_DURATION", 4)))
        resolution = os.environ.get("SCENEFORGE_OPENROUTER_VIDEO_RESOLUTION", "720p")
        generate_audio = _env_bool("SCENEFORGE_OPENROUTER_GENERATE_AUDIO", True)
        ...
        _emit_progress(progress, "video_task_created", "OpenRouter video generation task created", {"provider": "openrouter", "model": self.model, "base_url": self.base_url, "job_id": job_id, "polling_url": polling_url, "status": create_payload.get("status")})
    ...
    async def inspect_remote_task(
        ...
        *,
        ...
        polling_url = str(values.get("polling_url") or f"{self.base_url}/videos/{remote_task_id}")
        poll_url = _absolute_url(self.base_url, polling_url)
        status_code, payload = await _get_json(
            poll_url,
            headers=self._headers(),
            timeout=timeout,
            hard_timeout_seconds=request_timeout_seconds,
        ...
        if status_code >= 400:
            raise RuntimeError(f"OpenRouter video poll failed with HTTP {status_code}: {payload}")
        status = str(payload.get("status") or "unknown").lower()
        if status == "completed":
            urls = payload.get("unsigned_urls") or []
            content_url = str(urls[0]) if urls else f"{self.base_url}/videos/{remote_task_id}/content?index=0"
            content_url = _absolute_url(self.base_url, content_url)
            download_status, data = await _get_bytes(
                content_url,
                headers=self._headers() if _needs_authorization(content_url) else {},
                timeout=timeout,
                hard_timeout_seconds=request_timeout_seconds,
            )
            if download_status >= 400:
            ...
            return RemoteVideoInspection(
                ...
                status,
                output=VideoOutput(fmt="bytes", ext="mp4", data=data),
            )
        if status in {"failed", "cancelled", "expired"}:
            return RemoteVideoInspection(RemoteVideoState.FAILED, status, error=str(payload.get("error") or payload))
...
def _absolute_url(base_url: str, url: str) -> str:
    ...
    if url.startswith("/"):
        ...
        return urlunsplit((base.scheme, base.netloc, url, "", ""))

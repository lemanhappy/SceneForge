...
class VideoGeneratorVeoYunwuAPI:
    ...
    async def generate_single_video(
        ...
        **kwargs,
        ...
        create_retries = _env_int("SCENEFORGE_VIDEO_CREATE_RETRIES", 3)
        ...
        poll_interval_seconds = _env_float("SCENEFORGE_VIDEO_POLL_INTERVAL_SECONDS", 5.0)
        max_query_errors = _env_int("SCENEFORGE_VIDEO_MAX_QUERY_ERRORS", 5)
        ...
        for attempt in range(1, create_retries + 1):
            ...
            if task_id:
                # Persisting the remote handle is control flow, not part of the
                # provider POST retry. If the callback rejects a stale worker,
                # propagate it without submitting another paid task.
                _emit_progress(progress, "video_task_created", "Video generation task created", {"provider": "veo_yunwu", "model": model, "base_url": self.base_url, "task_id": task_id})
                break
        if not task_id:
    ...
    async def inspect_remote_task(
        ...
        *,
        ...
        timeout_seconds = _env_float("SCENEFORGE_VIDEO_REQUEST_TIMEOUT_SECONDS", 60.0)
        timeout = aiohttp.ClientTimeout(total=timeout_seconds)
        headers = {"Accept": "application/json", "Authorization": f"Bearer {self.api_key}"}
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(
                f"{self.base_url}/v1/video/query?id={remote_task_id}",
                headers=headers,
            ) as response:
                payload = await response.json(content_type=None)
                if response.status >= 400:

...
> **样片模型说明**
>
> 当前公开样片使用的是成本优先的低价模型配置，重点展示 SceneForge 的多镜头生产、资产复用、连续性控制和成片流程，并不代表系统可达到的最高画质。SceneForge 支持独立切换语言、图像和视频模型；接入能力更强的最新模型后，通常可以进一步改善人物一致性、画面细节、动作自然度和镜头稳定性，实际提升取决于所选模型与供应商。
